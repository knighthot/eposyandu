...
const EditIbuForm = () => {
  ...
  const route = useRoute(); // To receive the DataOrtu from navigation
...
const [errorMessages, setErrorMessages] = useState([]);
  ...
  useEffect(() => {
    ...
    // Populate the form with data if it exists in route params
    if (route.params?.DataOrtu) {
      setIbuData(route.params.DataOrtu);
    }
  ...
  const handleNext = () => {
    ...
    navigation.navigate('EditAyahForm', { ibuData: formattedIbuData});
    ...
    <View>
      <View style={styles.scene}>
        ...
        <TextInput
          ...
          maxLength={16}
          placeholderTextColor="#000"
          value={ibuData.nik_ibu.toString()}
        ...
        <DatePicker
          ...
          date={ibuData.tanggal_lahir_ibu ? new Date(ibuData.tanggal_lahir_ibu) : new Date()}
          mode="date"
          onConfirm={(date) => {
        ...
        <TextInput
          ...
          maxLength={12}
          value={ibuData.no_hp_ibu.toString()}
