...
const EditAyahForm = () => {
  ...
  const { ibuData } = route.params || {}; // Dapatkan ibuData dari route.params
const [errorMessages, setErrorMessages] = useState([]);
  ...
  useEffect(() => {
    // Cek jika ibuData ada, tampilkan di console dan bisa dipakai di UI
    if (ibuData) {
      console.log('Received ibuData:', ibuData);  // Tampilkan ibuData untuk verifikasi
    }

    const fetchPekerjaan = async () => {
      try {
        const response = await axios.get(`${Config.API_URL}/pekerjaan/`);
    ...
    const fetchPendidikan = async () => {
      try {
        const response = await axios.get(`${Config.API_URL}/pendidikan/`);
  ...
  console.log(ibuData, "data ibu");
    ...
    try {
      ...
      const orangTuaResponse = await axios.put(`${Config.API_URL}/orangtua/${ayahData.id}`, {
        no_kk: ibuData.no_kk,
    ...
    <View>
      <View style={styles.scene}>
        ...
        <TextInput
          ...
          value={ibuData.nik_ayah.toString()}
        ...
        <DatePicker
          ...
          date={ayahData.tanggal_lahir_ibu ? new Date(ayahData.tanggal_lahir_ibu) : new Date()}
          mode="date"
          onConfirm={(date) => {
        ...
        <TextInput
          ...
          maxLength={12}
          placeholderTextColor="#000"
          value={ayahData.no_hp_ayah.toString()}
